...
s.ARENA_HALF     = 0.99;     % inner face of the wall for a 2 m arena [m]; see labhalf
...
% --- robots --------------------------------------------------------------
% Real platforms with their published dimensions. The sensor, the controller
% and the requirements are identical for every one of them, so any difference
% in verdict is caused by the chassis and nothing else.
%
% radius = body half-width [m], track = axle width [m], wheel = wheel radius
% [m], base = wheelbase [m] and only meaningful for the steered models.
s.robots = i_robot('E_PUCK','e-puck','diff', 0.037, 0.052, 0.0205, 0, ...
    'Educational robot, 7.4 cm across. Turns on the spot.');
s.robots(2) = i_robot('TURTLEBOT3_BURGER','TurtleBot3 Burger','diff', 0.105, 0.160, 0.033, 0, ...
    'ROS teaching platform, 21 cm across. Turns on the spot.');
s.robots(3) = i_robot('TURTLEBOT3_WAFFLE','TurtleBot3 Waffle','diff', 0.153, 0.287, 0.033, 0, ...
    'Larger TurtleBot3, 31 cm across. Turns on the spot.');
s.robots(4) = i_robot('PIONEER_3DX','Pioneer 3-DX','diff', 0.22, 0.330, 0.0975, 0, ...
    'Research robot, 44 cm across. Turns on the spot, but it is wide.');
s.robots(5) = i_robot('CLEARPATH_JACKAL','Clearpath Jackal','diff', 0.33, 0.376, 0.098, 0, ...
    'Outdoor UGV, 66 cm across. Needs a bigger arena than the default.');
s.robots(6) = i_robot('CLEARPATH_HUSKY','Clearpath Husky','diff', 0.40, 0.555, 0.165, 0, ...
    'Large field robot, 80 cm across. Will not fit the default 2 m arena.');
s.robots(7) = i_robot('CAR_LIKE_SMALL','Car-like, small','car', 0.037, 0.052, 0.0205, 0.12, ...
    'e-puck sized but steered like a car: 30 degree limit, so it cannot pivot.');
s.robots(8) = i_robot('CAR_LIKE_LARGE','Car-like, large','car', 0.22, 0.330, 0.0975, 0.32, ...
    'Pioneer sized and steered like a car. Separates size from steering type.');
...
s.parameters = i_param('robot','enum','E_PUCK',0,0,0,'Robot','', ...
    ...
    {s.robots.key});
s.parameters(2) = i_param('arena_size','float',2.0,1.0,6.0,0.1,'Arena size','m', ...
    'Side length of the square arena. Every scenario scales with it.', {});
s.parameters(3) = i_param('scenario','enum','SINGLE_OBSTACLE',0,0,0,'Scenario','', ...
    'Which preconfigured arena layout to test against.', keys);
s.parameters(4) = i_param('speed','float',5.5,0.5,s.MAX_WHEEL_SPEED,0.01,'Wheel speed','rad/s', ...
    sprintf('Cruise wheel speed. %.2f rad/s is the motor limit (%.3f m/s).', ...
    s.MAX_WHEEL_SPEED, s.MAX_WHEEL_SPEED*s.WHEEL_RADIUS), {});
s.parameters(5) = i_param('start_x','float',-0.70,-3.0,3.0,0.01,'Start X','m', ...
    'Initial robot position, X axis.', {});
s.parameters(6) = i_param('start_y','float',-0.70,-3.0,3.0,0.01,'Start Y','m', ...
    'Initial robot position, Y axis.', {});
s.parameters(7) = i_param('start_heading_deg','float',45,-180,180,1,'Start heading','deg', ...
    'Initial orientation. 0 deg faces +X, 90 deg faces +Y.', {});
s.parameters(8) = i_param('goal_x','float',0.70,-3.0,3.0,0.01,'Goal X','m', ...
    'Target position, X axis.', {});
s.parameters(9) = i_param('goal_y','float',0.70,-3.0,3.0,0.01,'Goal Y','m', ...
    'Target position, Y axis.', {});
s.parameters(10) = i_param('obstacle_x','float',0,-3.0,3.0,0.01,'Obstacle X','m', ...
    'SINGLE_OBSTACLE only: centre of the cylinder, X axis.', {});
s.parameters(11) = i_param('obstacle_y','float',0,-3.0,3.0,0.01,'Obstacle Y','m', ...
    'SINGLE_OBSTACLE only: centre of the cylinder, Y axis.', {});
s.parameters(12) = i_param('corridor_width','float',0.40,0.10,5.0,0.01,'Corridor width','m', ...
    sprintf('CORRIDOR only: width of the gap. The robot is %.3f m wide.',2*s.ROBOT_RADIUS), {});
s.parameters(13) = i_param('sensor_noise','float',0,0,0.50,0.01,'Sensor noise','fraction', ...
    'Relative Gaussian noise on every range reading. Reproducible for a given seed.', {});
s.parameters(14) = i_param('max_time','float',30,5,120,1,'Max execution time','s', ...
    'Simulated seconds allowed before the run is declared a timeout.', {});
s.parameters(15) = i_param('seed','int',1,1,999999,1,'Noise seed','', ...
...
function o = i_robot(key, label, kind, radius, track, wheel, base, description)
o.key = key; o.label = label; o.kind = kind; o.radius = radius;
o.track = track; o.wheel = wheel; o.base = base; o.description = description;
end
