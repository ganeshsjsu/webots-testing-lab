...
s.parameters = i_param('scenario','enum','SINGLE_OBSTACLE',0,0,0,'Scenario','', ...
    'Which preconfigured arena layout to test against.', keys);
s.parameters(2) = i_param('speed','float',5.5,0.5,s.MAX_WHEEL_SPEED,0.01,'Wheel speed','rad/s', ...
    sprintf('Cruise wheel speed. %.2f rad/s is the motor limit (%.3f m/s).', ...
    s.MAX_WHEEL_SPEED, s.MAX_WHEEL_SPEED*s.WHEEL_RADIUS), {});
s.parameters(3) = i_param('start_x','float',-0.80,-0.85,0.85,0.01,'Start X','m', ...
    'Initial robot position, X axis.', {});
s.parameters(4) = i_param('start_y','float',-0.80,-0.85,0.85,0.01,'Start Y','m', ...
    'Initial robot position, Y axis.', {});
s.parameters(5) = i_param('start_heading_deg','float',45,-180,180,1,'Start heading','deg', ...
    'Initial orientation. 0 deg faces +X, 90 deg faces +Y.', {});
s.parameters(6) = i_param('goal_x','float',0.80,-0.85,0.85,0.01,'Goal X','m', ...
    'Target position, X axis.', {});
s.parameters(7) = i_param('goal_y','float',0.80,-0.85,0.85,0.01,'Goal Y','m', ...
    'Target position, Y axis.', {});
s.parameters(8) = i_param('obstacle_x','float',0,-0.85,0.85,0.01,'Obstacle X','m', ...
    'SINGLE_OBSTACLE only: centre of the cylinder, X axis.', {});
s.parameters(9) = i_param('obstacle_y','float',0,-0.85,0.85,0.01,'Obstacle Y','m', ...
    'SINGLE_OBSTACLE only: centre of the cylinder, Y axis.', {});
s.parameters(10) = i_param('corridor_width','float',0.40,0.15,0.90,0.01,'Corridor width','m', ...
    sprintf('CORRIDOR only: width of the gap. The robot is %.3f m wide.',2*s.ROBOT_RADIUS), {});
s.parameters(11) = i_param('sensor_noise','float',0,0,0.50,0.01,'Sensor noise','fraction', ...
    'Relative Gaussian noise on every range reading. Reproducible for a given seed.', {});
s.parameters(12) = i_param('max_time','float',30,5,120,1,'Max execution time','s', ...
    'Simulated seconds allowed before the run is declared a timeout.', {});
s.parameters(13) = i_param('seed','int',1,1,999999,1,'Noise seed','', ...
