...
% --- robots --------------------------------------------------------------
s.robots = i_scen('DIFFERENTIAL_DRIVE','Differential drive (e-puck)', ...
    'Two independently driven wheels. Can turn on the spot.', {});
s.robots(2) = i_scen('UNICYCLE','Unicycle', ...
    'Idealised model with no per-wheel speed coupling. Also turns on the spot.', {});
s.robots(3) = i_scen('CAR_LIKE','Car-like (steered front wheel)', ...
    ['Steered front wheel with a 30 degree limit, so it has a minimum ' ...
     'turning radius of about 0.21 m and cannot turn on the spot.'], {});

% --- input parameters ----------------------------------------------------
keys = {s.scenarios.key};
s.parameters = i_param('robot','enum','DIFFERENTIAL_DRIVE',0,0,0,'Robot','', ...
...
s.parameters(4) = i_param('start_x','float',-0.80,-0.85,0.85,0.01,'Start X','m', ...
    'Initial robot position, X axis.', {});
s.parameters(5) = i_param('start_y','float',-0.80,-0.85,0.85,0.01,'Start Y','m', ...
...
s.parameters(7) = i_param('goal_x','float',0.80,-0.85,0.85,0.01,'Goal X','m', ...
    'Target position, X axis.', {});
s.parameters(8) = i_param('goal_y','float',0.80,-0.85,0.85,0.01,'Goal Y','m', ...
